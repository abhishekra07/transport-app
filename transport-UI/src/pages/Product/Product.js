...
const headers = [
  ...
  { label: "Product Image", key: "productImg" },
...
function Product() {
  ...
  const columns = [
    ...
    {
      name: "Product Image",
      cell: (row) => <img src={row.productImg} alt={row.productName} />,
      reorder: true,
    },
    {
      name: "Action",
  ...
  useEffect(() => {
    const result = ProductData.filter((row) => {
      return row.productName.toLowerCase().includes(search.toLowerCase());
    });
